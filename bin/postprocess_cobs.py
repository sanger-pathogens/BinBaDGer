...
def step_and_print_lines(lines, hits_to_keep, show_count, output_file, seed):
    ...
    if hits_to_keep > len(lines):
        hits_to_keep = len(lines)

    total_lines = len(lines)

    if hits_to_keep == 1:
        # For single hit, take the middle element
        selected_indices = [total_lines // 2]
    else:
        # Calculate the step size to ensure even distribution
        # Subtract 1 from hits_to_keep to account for including both start and end
        step = (total_lines - 1) / (hits_to_keep - 1)
        selected_indices = [round(i * step) for i in range(hits_to_keep)]
...
def main():
    ...
    parser.add_argument(
        ...
        default=500,
        type=int,
        help="Number of top hits to keep, default: 500 \
        if set to much higher number there could not be enough possible matches, \
