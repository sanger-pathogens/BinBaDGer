...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bins",
        type=str,
        default="0.80,0.95,0.99,0.995,0.998,1",
        help="Comma-separated list of bin edges, e.g., '0.98,0.99,0.995,0.998,1'.",
    )
    parser.add_argument(
        "--assign_outsiders",
        action="store_true",
        help="add a new bin to the lower side to catch those which fall below the lowest edge",
    )
    parser.add_argument(
        "--log_file", type=str, default="binning.log", help="Path to save log file for binning warnings"
    )
