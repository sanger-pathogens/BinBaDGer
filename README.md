...
| `--selection_method`       | `string`  | `top`                                                        | Candidate selection method; `top`: top N matches based on coverage, `stepwise`: return N evenly spaced matches chosen randomly, or `random`: N random matches, to capture diversity in a small set. A seed is set so that random matches are reproducible.                                                                   |
